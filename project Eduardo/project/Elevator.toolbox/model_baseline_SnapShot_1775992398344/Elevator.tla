...
(***************************************************************************)
(* Property 1: An elevator must never be in motion if its doors are open.  *)
(***************************************************************************)
DoorsClosedWhileMoving == 
    \A e \in Elevator : 
        ElevatorState[e].direction /= "Stationary" => ~ElevatorState[e].doorsOpen
        
        
(***************************************************************************)
(* Property 2: Coordination Safety                                         *)
(* Two different elevators must never have their doors open on the same    *)
(* floor simultaneously.                                                   *)
(***************************************************************************)
NoDualDoorOpen == 
    \A e1, e2 \in Elevator : 
        (e1 /= e2 /\ ElevatorState[e1].doorsOpen /\ ElevatorState[e2].doorsOpen)
        => ElevatorState[e1].floor /= ElevatorState[e2].floor
        
(***************************************************************************)
(* Property 3: Guaranteed Boarding (Liveness)                              *)
(* If a person is in a waiting state, they will eventually enter an        *)
(* elevator car. This ensures that no passenger is ignored indefinitely    *)
(* (starvation) and relies on the Strong Fairness of the dispatcher.       *)
(***************************************************************************)
EventuallyPickedUp == 
    \A p \in Person : 
        PersonState[p].waiting ~> (PersonState[p].location \in Elevator)
...
\* Last modified Sun Apr 12 12:12:49 WEST 2026 by eamachado
\* Created Thu Apr 09 15:01:48 WEST 2026 by eamachado
